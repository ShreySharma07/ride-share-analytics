...
import pandas as pd
from business_metrics import calculate_kpi, processed_data
...
def load_data(file_path):
    try:
        ...
        st.error(f"Error: Processed data file not found at '{file_path}'.")
        return None

@st.cache_resource # Cache the ML model
def get_model(X, y):
    return train_evaluate_model(X, y)

# --- Data Loading and Preparation ---
df_raw = load_data(processed_data)

# --- Title ---
st.title("Ride-Share Business Analytics Dashboard ")
st.markdown("This dashboard provides an overview of KPIs, fraud detection, and statistical analyses for NYC Yellow Taxis.")
...
if df_raw is not None:
    ...
    # Join predictions back to the original data with all columns
